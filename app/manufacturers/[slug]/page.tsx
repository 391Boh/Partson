...
const getProducerBySlug = cache(async (slug: string) => {
  ...
  const [producer, seoProducer] = await Promise.all([
    Promise.resolve(
      producerDirectory.find((directoryProducer) => directoryProducer.slug === normalizedSlug) ||
        null
    ),
    findSeoProducerBySlug(normalizedSlug).catch(() => null),
  ]);
  if (!producer) return null;

  return {
    ...producer,
    productCount: seoProducer?.productCount ?? 0,
    topGroups: seoProducer?.topGroups || [],
  };
});
...
const buildProducerDescription = (label: string, productCount: number) =>
  productCount > 0
    ...
    : `Автозапчастини ${label} у каталозі PartsON з переходом до товарів виробника і доставкою по Україні.`;
...
export async function generateMetadata({ params }: ProducerPageProps): Promise<Metadata> {
  ...
  return buildPageMetadata({
    title: `Автозапчастини ${producer.label} - каталог виробника`,
    description,
    canonicalPath,
    keywords: [
      producer.label,
    ...
    openGraphTitle: `Автозапчастини ${producer.label} | PartsON`,
    image: {
      url: "/Car-parts-fullwidth.png",
      alt: `Автозапчастини ${producer.label} | PartsON`,
    },
  });
}
...
export default async function ProducerDetailPage({ params }: ProducerPageProps) {
  ...
  const seoCopy =
    producer.productCount > 0
      ...
      : {
          title: `Автозапчастини ${producer.label} — каталог виробника у PartsON`,
          intro:
            producer.description ||
            `Сторінка бренду ${producer.label} допомагає швидко перейти до каталогу виробника і знайти потрібні автозапчастини.`,
          paragraphs: [
            `Бренд ${producer.label} винесений в окремий маршрут, щоб користувач міг одразу перейти до фільтрованого каталогу автозапчастин без зайвих кроків.`,
            `Маршрут зручний для навігації, швидкого переходу в каталог і прямого підбору автозапчастин бренду ${producer.label}.`,
          ],
          highlights: [
            `каталог автозапчастин бренду ${producer.label};`,
  ...
  const jsonLd = {
    ...
    "@type": "CollectionPage",
    name: `Автозапчастини ${producer.label} - каталог виробника`,
      ...
      <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="font-display-italic text-3xl tracking-[-0.048em] text-slate-900">
              Автозапчастини {visibleProducerLabel} - каталог виробника
            </h1>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
              {replaceProducerLabel(
                producer.description ||
                  `Сторінка бренду ${producer.label} зібрана як швидкий вхід у каталог виробника: звідси зручно перейти до фільтрованих товарів і купівлі автозапчастин бренду.`
              )}
            </p>
