...
const buildGroupDescription = (label: string, productCount: number) =>
  `Група товарів "${label}" у каталозі PartsON. Доступно товарів: ${productCount}. Перейдіть до каталогу для підбору і замовлення.`;

export async function generateMetadata({ params }: GroupPageProps): Promise<Metadata> {
  const { slug } = await params;
  const group = await findSeoGroupBySlug(slug);
  if (!group) {
    return {
...
export default async function GroupDetailPage({ params }: GroupPageProps) {
  ...
  return (
    <main className="mx-auto w-full max-w-[1100px] px-4 py-8">
      ...
      <p className="mt-2 text-sm text-slate-600">Знайдено товарів: {group.productCount}</p>
