      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-slate-700">
              {hasPrice
                ? "Додавайте товар у замовлення прямо зі сторінки."
                : "Сформуйте готовий запит менеджеру по цьому товару."}
            </p>
          </div>
          {cartQty > 0 && hasPrice && (
            <span className="inline-flex rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.08em] text-emerald-700">
              У кошику: {cartQty}
            </span>
          )}
        </div>
        ...
        {hasPrice ? (
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="inline-flex w-fit items-center rounded-2xl border border-slate-200 bg-slate-50 p-1 shadow-sm">
              <button
                type="button"
