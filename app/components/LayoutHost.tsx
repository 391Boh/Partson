...
export default function LayoutHost({ children }: LayoutHostProps) {
  ...
  const [AccountRuntime, setAccountRuntime] =
    useState<ComponentType<LayoutAccountRuntimeProps> | null>(null);
  ...
  const requestAccountRuntime = useCallback(() => {
    void import("./LayoutAccountRuntime")
      .then((module) => setAccountRuntime(() => module.default))
      .catch((error) => {
        console.error("Failed to load account runtime:", error);
      });
  }, []);
  const openChat = useCallback(() => {
    requestAccountRuntime();
    setIsChatOpen(true);
  }, [requestAccountRuntime]);
  ...
  useEffect(() => {
    if (typeof window === "undefined" || AccountRuntime) return;
  ...
  }, [AccountRuntime, pathname, requestAccountRuntime]);
  ...
  useEffect(() => {
    if (!firebaseUser || AccountRuntime) return;
    requestAccountRuntime();
  }, [AccountRuntime, firebaseUser, requestAccountRuntime]);
  ...
  // Speculative route downloads must follow the current page's resources.
  ...
  useEffect(() => {
    ...
    const cancelWarmup = scheduleBackgroundTask(
      () => {
        if (shouldUseLightNetworkWarmup()) return;
        didWarmRoutes = true;
      ...
      { delayMs: pathname === "/" ? 3200 : 700, idleTimeoutMs: 4000 }
    );
    ...
    <div style={{ ["--header-height" as string]: "4rem" }}>
      ...
      {AccountRuntime && (
        <AccountRuntime
          ...
          setUserUnreadCount={setUserUnreadCount}
