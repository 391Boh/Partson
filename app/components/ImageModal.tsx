...
const withRetryToken = (value: string, retryToken: number) => {
  const trimmed = (value || "").trim();
  if (!trimmed) return "";

  try {
    const url = new URL(trimmed, "http://localhost");
    url.searchParams.set("retry", String(retryToken));
  ...
  const modalNode = (
    ...
    >
      ...
      >
        ...
        >
          {loadState !== "loaded" ? (
            <div className="absolute inset-0 z-[1] flex items-center justify-center bg-[radial-gradient(circle_at_center,rgba(30,41,59,0.88)_0%,rgba(2,6,23,0.98)_78%)]">
              {loadState === "failed" ? (
                <div className="flex flex-col items-center gap-2 text-center text-white/78">
                  <span className="text-sm font-semibold">Фото не вдалося завантажити</span>
                  <span className="text-xs text-white/55">
                    Спробуйте відкрити товар ще раз трохи пізніше
                  </span>
                </div>
              ) : (
                <div className="h-10 w-10 animate-pulse rounded-full border border-white/18 bg-white/10" />
              )}
            </div>
          ) : null}
          ...
          <img
            ...
            onError={() => {
              if (!retryAttempted) {
                const retrySrc = withRetryToken(displaySrc, 1);
                if (retrySrc && retrySrc !== displaySrc) {
                  setRetryAttempted(true);
                  setLoadState("loading");
                  setDisplaySrc(retrySrc);
                  return;
                }
              }
