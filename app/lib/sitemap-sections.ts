...
const buildGroupSitemapImage = (options: {
  categoryLabel: string;
  itemLabel?: string;
  parentLabel?: string;
}) => {
  const preview = getGroupProductPreview(options);

  return {
    // A verified catalog-product photo is the preferred image. The category
    // icon remains a safe fallback only for a newly added tree node whose
    // preview has not been generated by the next build yet.
    loc: preview?.url || getCategoryIconPath(options.categoryLabel),
  };
};
...
const collectGroupListingPaths = (
  ...
  const subgroupPaths: Array<{
    path: string;
    label: string;
    categoryLabel: string;
    parentLabel: string;
  }> = [];
  const childPaths: Array<{
    path: string;
    label: string;
    categoryLabel: string;
    parentLabel: string;
  ...
  for (const group of groups) {
    ...
    for (const subgroup of group.subgroups || []) {
      ...
      if (
        (!onlyWithProducts || (subgroup.productCount || 0) > 0) &&
        !seenSubgroupPaths.has(subgroupPath)
      ) {
      ...
      for (const child of Array.isArray(subgroup.children) ? subgroup.children : []) {
        ...
        if (onlyWithProducts && (child.productCount || 0) <= 0) continue;
        ...
        childPaths.push({
          ...
          categoryLabel: group.label,
          parentLabel: subgroup.label || group.label,
        });
      }
    }
  }
...
const buildGroupsSitemapEntries = async (): Promise<SitemapPathEntry[]> => {
  ...
  const directoryData = await getFullGroupsDirectoryData().catch(() => null);
  if (!directoryData || directoryData.clientGroups.length === 0) return entries;

  // When the catalog snapshot contains product counts, submit only useful
  // landing pages. Empty tree nodes remain navigable on the site with
  // noindex/follow, but don't consume crawl budget or compete with populated
  // categories. If counts are temporarily unavailable, keep the complete
  // tree as a fail-open fallback rather than dropping URLs on an outage.
  // Keep the complete group tree discoverable. Pages without a generated
  // product preview still receive their parent category icon via
  // buildGroupSitemapImage, so they must not disappear from the sitemap.
  const onlyWithProducts = false;
  const sitemapGroups = directoryData.clientGroups.filter(
    (group) => !onlyWithProducts || group.productCount > 0
  );

  for (const group of sitemapGroups.slice(0, maxGroupPages)) {
    entries.push({
      path: `/groups/${encodeURIComponent(group.slug)}`,
      lastModified: contentLastModified,
      changeFrequency: "weekly",
      priority: 0.84,
      images: [buildGroupSitemapImage({ categoryLabel: group.label })],
    });
  }

  const { subgroupPaths, childPaths } = collectGroupListingPaths(
    sitemapGroups,
    onlyWithProducts
  );
...
const getGroupsSitemapEntriesCached = unstable_cache(
  ...
  ["groups-sitemap-v4-product-previews"],
  {
