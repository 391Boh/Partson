...
export async function GET() {
  try {
    const cacheResponse = await oneCRequest("GetCache", {
      method: "GET",
      retries: 1,
      cacheTtlMs: 1000 * 2,
    ...
    const cacheHash =
      cacheResponse.status >= 200 && cacheResponse.status < 300
        ? resolveCatalogVersionHash(cacheResponse.text)
        : null;
    ...
    const catalogPayload = normalizeString(catalogResponse.text);
    const catalogHash =
      catalogResponse.status >= 200 && catalogResponse.status < 300 && catalogPayload
        ? buildHash(catalogPayload)
        : null;
