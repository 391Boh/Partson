...
const buildStaticPages = (siteUrl: string, lastModified: Date): SitemapEntry[] => [
  ...
  {
    url: toAbsoluteSitePath(siteUrl, "/inform/delivery"),
    lastModified,
    changeFrequency: "monthly",
    priority: 0.72,
  },
  {
    url: toAbsoluteSitePath(siteUrl, "/inform/payment"),
    lastModified,
    changeFrequency: "monthly",
    priority: 0.62,
  },
  {
    url: toAbsoluteSitePath(siteUrl, "/inform/about"),
    lastModified,
    changeFrequency: "monthly",
    priority: 0.62,
  },
  {
    url: toAbsoluteSitePath(siteUrl, "/inform/location"),
    lastModified,
    changeFrequency: "monthly",
    priority: 0.62,
  },
...
export async function GET() {
  ...
  const maxManufacturerGroupPages = parsePositiveInt(
    process.env.SITEMAP_MAX_MANUFACTURER_GROUP_PAGES,
    30000
  );
  ...
  try {
    ...
    for (const group of facets.groups.slice(0, maxGroupPages)) {
      pushUniqueEntry(entries, seenUrls, {
        url: toAbsoluteSitePath(siteUrl, `/groups/${encodeURIComponent(group.slug)}`),
    ...
    for (const producer of facets.producers.slice(0, maxManufacturerPages)) {
      pushUniqueEntry(entries, seenUrls, {
        url: toAbsoluteSitePath(
          siteUrl,
          `/manufacturers/${encodeURIComponent(producer.slug)}`
        ),
        lastModified: facetLastModified,
        changeFrequency: "weekly",
        priority: 0.82,
    ...
    let producerGroupCount = 0;
    for (const producer of facets.producers) {
      if (producerGroupCount >= maxManufacturerGroupPages) break;

      for (const group of producer.topGroups) {
        if (producerGroupCount >= maxManufacturerGroupPages) break;

        const added = pushUniqueEntry(entries, seenUrls, {
          url: toAbsoluteSitePath(
            siteUrl,
            buildCatalogProducerPath(producer.label, group.label)
          ),
          lastModified: facetLastModified,
          changeFrequency: "weekly",
          priority: 0.76,
